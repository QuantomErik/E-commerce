...
const CartDrawer = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      ></div>
      <div
        className={`fixed top-0 right-0 z-40 cart-drawer h-screen p-4 overflow-y-auto transition-transform duration-500 ${isOpen ? 'translate-x-0' : 'translate-x-full'} bg-white w-96 border border-gray-300`}
        tabIndex="-1"
            ...
            <ul>
              {cart.map((product) => (
                <li key={product.id} className="flex items-center justify-between my-2">
                  <img src={product.image_url} alt={product.name} className="w-12 h-12 object-cover mr-2" />
                  <div className="flex-grow">
                    {product.name} - ${product.discountedPrice ? product.discountedPrice.toFixed(2) : product.price.toFixed(2)}
                  </div>
                  <div className="flex items-center space-x-2">
                    <div className="border border-gray-300 rounded p-1 flex items-center">
                      <button onClick={() => updateQuantity(product.id, product.quantity - 1)} className="text-gray-500 mr-2 ml-1">
                    ...
                    <div>
                      {product.discountedPrice && product.discountedPrice !== product.price ? (
                        <>
                          ...
                          <span className="font-bold text-red-500 ml-2">${product.discountedPrice}</span>
                        </>
                      ) : (
